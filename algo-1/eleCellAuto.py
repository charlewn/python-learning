...
# ... 1, ..
def cellular_automaton(seq, pattern, line):
	if pattern > 255:
		return None
	
	compose_number = [128, 64, 32, 16, 8, 4, 2, 1]
	pattern_dict = {"...":1,"..x":2, ".x.":4,".xx":8,"x..":16,"x.x":32,"xx.":64,"xxx":128}
	pattern_number = []
	
	for c in compose_number:
		if pattern >= c:
			pattern = pattern - c
			pattern_number.append(c)
	...
	while line != 0:
		for s in range(len_pattern):
			if s == 0:
				if len_pattern == 1:
					key = seq[s-1]+seq[s]+seq[s]
				else:
					key = seq[-1]+seq[0:2]
			elif s == len_pattern-1:
				key = seq[len_pattern-2:len_pattern]+seq[0]
			else:
				key = seq[s-1:s+2]
			
			if pattern_dict[key] in pattern_number:
				next_line+="x"
			else:
				next_line+="."
		seq = next_line
